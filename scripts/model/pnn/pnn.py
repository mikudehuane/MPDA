...
class PNN(ModelFrame):
    ...
    def forward(self, features, mode):
        ...
        # 这里数组长度会变，但是构建迭代器时为初始长度
        with tf.name_scope('product'):
            for field1_idx in range(len(forward_net_inps) - 1):
                field1 = forward_net_inps[field1_idx]
                for field2_idx in range(field1_idx + 1, len(forward_net_inps)):
                    field2 = forward_net_inps[field2_idx]
                    # dot product
                    product_input = field1 * field2
                    forward_net_inps.append(product_input)
        ...
        logits = forward_net(forward_net_inps, name='forward_net', mode=_mode)
