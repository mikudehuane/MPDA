...
init_model_fp="{project_fp}/cloud_models/amazon_din/checkpoint/epoch/model.ckpt"
...
device="cpu"
model="din"
run_name="transfer_amazon_din_m-${max_match}"

mkdir -p ../log/${run_name}/running-logs/

for ti in $(seq 0 1 14)
do
  command="../scripts/transfer.py --device=${device} -ti=${ti} -tc=15 -rn=${run_name} -mo=${model} -mm=${max_match} -lr=0.01 -ma=random -rands=${random_seed} -uas=0 -imf=${init_model_fp} -ds=amazon --mapping_fp=${mapping_fp} -tdf=${train_data_fd} -edf=${eval_data_fd} -eulp=${examine_user_list_fp} -aup=${examine_user_list_fp}"
